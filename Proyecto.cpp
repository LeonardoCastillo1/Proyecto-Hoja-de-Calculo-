...
//Creamos la funcion de Crear Fila y Columna que tambien buscara si ya hay una en esa posicion
    ...
    void EliminarNodoFilaSinCelda(Matriz* &fila){
        if(fila==nullptr || fila->cel!=nullptr)return;//Si fila no existe o tiene celda retorna
        EliminarNodoFilaHash(fila->index);
        if(head->row==fila){//Si es el elemento despues del head
            head->row=fila->sig;
            delete fila;
            fila=nullptr;
            return;
        }
        Matriz* prevfila=head->row;
        while(prevfila!=nullptr && prevfila->sig != fila){
            prevfila=prevfila->sig;
        }
        if (prevfila!=nullptr){
            prevfila->sig=fila->sig;
            delete fila;
            fila=nullptr;
            return;
        }
    }
        void EliminarNodoColumnaSinCelda(Matriz* &columna){
        if(columna==nullptr || columna->cel!=nullptr)return;//Si fila no existe o tiene celda retorna
        EliminarNodoColumnaHash(columna->index);
        if(head->col==columna){//Si es el elemento despues del head
            head->col=columna->sig;
            delete columna;
            columna=nullptr;
            return;
        }
        Matriz* prevcolumna=head->col;
        while(prevcolumna!=nullptr && prevcolumna->sig != columna){
            prevcolumna=prevcolumna->sig;
        }
        if (prevcolumna!=nullptr){
            prevcolumna->sig=columna->sig;
            delete columna;
            columna=nullptr;
            return;
        }
    ...
    void EliminarCelda(Celdas* &Celda, Matriz* &Fila,Matriz* &Col){//Obtener celda una vez tenemos la ubicac
        ...
        Celda=nullptr;
        if(Fila->cel==nullptr){
            EliminarNodoFilaSinCelda(Fila);
        }
        if(Col->cel==nullptr){
            EliminarNodoColumnaSinCelda(Col);
        }
    }
    ...
    void EliminarRangoDCeldas(int ExtremoFilA,int ExtremoFilB,int ExtremoColA,int ExtremoColB){
        if (ExtremoFilA>ExtremoFilB){
            int B=ExtremoFilB;
            ExtremoFilB=ExtremoFilA;
            ExtremoFilA=B;
        }
        if (ExtremoColA>ExtremoColB){
            int B=ExtremoColB;
            ExtremoColB=ExtremoColA;
            ExtremoColA=B;
        }
        for(int fila=ExtremoFilA;fila<=ExtremoFilB;++fila){
            
            Matriz* PtrFila = BuscarFilaHash(fila);
            if(PtrFila==nullptr)continue;//Salta la iteracion si se cumple la condicion
            Celdas* temp=PtrFila->cel;
            while(temp!=nullptr && temp->col <ExtremoColA){
                temp=temp->next;
            }
            while(temp!=nullptr && temp->col <=ExtremoColB){
                Celdas* tempnext=temp->next;
                Matriz* PtrCol= BuscarColumnaHash(temp->col); 
                EliminarCelda(temp,PtrFila,PtrCol);

                if(PtrFila==nullptr)break;
                temp=tempnext;
